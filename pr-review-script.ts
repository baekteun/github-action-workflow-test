...
const T = c.getInput("GITHUB_TOKEN"),
  M = c.getInput("OLLAMA_MODEL");

const o = new O({ auth: T }),
  l = new L();

async function _() {
  const { repository: e, number: n } = JSON.parse(
    r(process.env.GITHUB_EVENT_PATH || "", "utf8")
  );
  const t = await o.pulls.get({
    owner: e.owner.login,
    repo: e.name,
    pull_number: n,
  });
