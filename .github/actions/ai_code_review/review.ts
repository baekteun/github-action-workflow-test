...
export const codeReview = async (
  ...
  const ollamaConcurrencyLimit = pLimit(options.ollamaConcurrencyLimit);
...
`;
  ...
  const doSummary = async (
    ...
    try {
      ...
      if (summarizeResp === "") {
        info("summarize: nothing obtained from Ollama");
      ...
      warning(`summarize: error from Ollama: ${e as string}`);
      summariesFailed.push(`${filename} (error from Ollama: ${e as string})})`);
      return null;
    }
  };
...
`;
      ...
      if (summarizeResp === "") {
        warning("summarize: nothing obtained from Ollama");
      } else {
  ...
  if (options.disableReleaseNotes === false) {
    ...
    if (releaseNotesResponse === "") {
      info("release notes: nothing obtained from Ollama");
    } else {
...
`;
      ...
      if (patchesPacked > 0) {
        ...
        try {
          ...
          if (response === "") {
            info("review: nothing obtained from Ollama");
